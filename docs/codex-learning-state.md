...
- 根因边界：新增的运行时计数确认 `2088` 已收到媒体包、但未形成任何播放器解码输入帧。当前直播播放器只接受 RTP/PT=96 的 H.265 数据，并需要 VPS/SPS/PPS 与关键帧；因此故障位于该摄像头的上游编码、RTP 封装或工控机转发，而不是二壮路由、SSO、门店权限、WSS 建连或浏览器整体 H.265 能力。
