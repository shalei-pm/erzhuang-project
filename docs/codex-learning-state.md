...
### 2026-08-26 3.1.12 回放定位复盘与 3.1.13 修复（待测试发布）
...
- 回归：新增 `buildNVRLabPlaybackSession` 测试，断言拖动后保留原窗口；前端 Vitest `10 files / 61 tests` 与生产构建通过。待推送测试环境后，必须用中段和靠近末尾两个位置验证：滑块比例保持、首帧恢复、游标继续推进。
