...
- 已将方案 A 代码推送到 GitLab 测试分支 `codex/containerize-single-image`，远端提交为 `670cd32`；未修改 `main`，未操作正式环境。
- 测试健康入口可达，但未登录命令行请求返回公司 SSO `302`；当前环境未暴露已登录 Chrome 插件，因此暂未确认 Wharf 752 的构建/部署记录和页面版本。
- 前端本地门禁已通过；后端 Go/MySQL 测试需在 Wharf 或明确隔离测试库执行后再补记录。
...
### 2026-08-27 3.2.16 NVR 首帧分类诊断

- 问题复现：测试门店 `10042` 的摄像头 `2088`（治疗室9）可创建取流会话并建立 WSS 连接，但 12 秒内没有渲染首帧；同一浏览器、同一门店的摄像头 `2089` 正常触发首帧并播放。
- 初步根因边界：`2088` 已收到媒体包，但当时的直播 H.265 直连路径没有递增 `decoderInputFrames`，该计数无法区分上游格式问题与浏览器解码问题。此口径已在 `3.2.17` 更正，不以本条初步结果对接口方下结论。
- 用户可见处理：直播 12 秒无首帧时分类提示为“未收到媒体数据”“未识别出可播放的视频帧”或“浏览器无法解码”，不暴露 token、WSS URL 或传输细节；回放链路不受影响。
- 测试发布：`543eafc fix: classify nvr first frame timeouts` 已同步 GitHub 和 GitLab 测试分支；Wharf `752` 构建 `169473` 成功（1 分 50 秒），自动部署记录 `415615` 已部署到阿里云测试集群。Chrome 已对 `2088` 验收得到“视频流已收到，但未识别出可播放的视频帧”，页面控制台无项目错误。

### 2026-08-27 3.2.17 NVR 直播帧链路细分

- 修正直播 H.265 直连路径遗漏的解码输入计数，并依次观测：二进制媒体包、有效 RTP、PT=96 视频包、VPS/SPS/PPS、H.265 关键帧、送入 WebCodecs 解码器和渲染首帧。
...
### 2026-09-01 3.3.0 操作日志

- 系统设置新增“操作日志”二级菜单，仅管理员可见；当前不新增审计人员角色，后续可通过独立 `audit.view` 权限扩展。
- 后端复用既有 `tb_audit_logs`，支持按最近 7 天默认范围、操作人、操作类型和日期查询；单次日期范围最多 3 个月。登录、退出、监控直播/回放、截图访问/刷新及用户权限变更均写入审计事件。
...
### 2026-09-02 3.4.1 测试镜像构建修复

- `3.4.0` 测试镜像构建失败的根因是 `internal/app/idle_session_test.go` 将通用 `Store` 接口直接当作审计查询存储使用，导致 Dockerfile 的 `go test ./...` 无法编译。
...
### 2026-09-02 3.4.4 截图水印透明度调整

- 根据截图验收反馈，监控截图右上角水印的深色底板从 `68%` 不透明度调整为 `30%`，减少对视频画面的遮挡；文字使用 `90%` 白色，保留姓名和服务端时间的清晰可读性。
- 未调整水印开关、内容、截图审计、权限或服务端时间来源；新增前端单测固定 `30%` 底板透明度。发布目标仅为 GitLab 测试分支 `codex/containerize-single-image` / Wharf `752`。

### 2026-09-02 3.4.6 安全合规修复测试发布

- 测试分支提交：`6d9ed67`，版本 `3.4.6`；包含旧版门店空间接口权限收紧、导出管理员限制与审计前置、移除萤石云诊断入口、HTTP 超时/请求体限制、安全响应头、健康检查脱敏和跨站退出防护。
- 验证：前端 Vitest `16 files / 93 tests`、生产构建、Go 定向审计回归、`go build ./...` 和 `git diff --check` 通过。Go 全量测试在本机还受到沙箱禁止绑定 IPv6 `::1` 的两个既有 NVR HTTP 测试限制；Wharf Linux 构建应继续执行完整测试。
- 发布：已同步 GitHub 备份分支和公司 GitLab 测试分支 `codex/containerize-single-image`；测试页自动部署完成，Chrome 复核页面显示 `3.4.6 (container)`，机构列表正常加载，萤石云诊断文案/入口不存在。未操作正式 `main`、正式环境或数据库。
- 验收范围：低权限跨门店接口运行时回归按产品决定移出本次验收；网关侧 APISIX/Ingress 的 WAF、限流和 WebSocket 限制仍由运维按基础设施范围核对。

### 2026-09-03 3.4.7 监控审计目标补全（待发布）

- NVR 监控的直播、回放和播放器截图审计，改为由服务端已校验的摄像头列表固化门店名称、机构 ID、空间类型、空间名称、摄像头名称和摄像头 ID；不接受前端提交的名称字段，也不记录流地址或鉴权信息。
- 新日志摘要示例：`门店：北京保利总部店（10001）；摄像头区域：治疗室 / 产研中心1-2；摄像头：治疗室4（ID：111）`。未绑定空间统一展示为“未绑定空间”。历史日志保持原样，不做回写。
...
### 2026-09-08 3.4.12 正式 main 已推送

- 用户确认测试登录回跳实验验收通过，并明确授权正式 main 推送，要求后续使用紧急发布方式。
- GitLab main 已从 `e6c5ab8` 正常前进到 `bf41906`（3.4.12）；只同步登录原页面恢复改动，保留正式 Cookie 清理行为，不修改数据库或实例配置。上一正式提交为回滚基线。
- 发布前重跑 Go 全仓测试、Linux 构建、前端 119 项测试及生产构建，全部通过。GitHub 对应开发分支已备份，未覆盖分叉的 GitHub main。
- 当前仅确认正式代码推送成功，Wharf 771 构建和紧急发布审批、部署尚待确认。测试部署记录为 418922（测试提交 ab9e4bba）。

### 2026-09-09 数字孪生 4.0.0 本地集成

- 在 `codex/digital-twin-module` 迁入本地数字孪生运行组件；源项目不变。新增 `/digitaltwin/` 登录保护入口及系统设置「数字孪生白名单」，缺省为10001北京保利总部店，与原用户机构权限取交集。
- 白名单复用 `tb_app_settings`，事务保存并审计、版本冲突返回409；无需DDL，未操作真实数据库。
- 摄像头按空间元数据映射：前台优先护士站其次；咨询室对应面诊室；等候区收录等候区和等待区；治疗室收录治疗室。术后护理与未匹配项暂不显示，不影响原监控目录。
- 点击摄像头在React弹窗复用NVRLabPlayer，只请求live；关闭卸载播放器、停止连接，重试重新申请会话。截图复用现有水印元数据及审计，不走无水印默认下载。
- 本地验证：前端122项测试、Go全仓测试、前端生产构建、Linux服务编译、diff检查通过；7组无头浏览器场景通过（桌面1440/1366、手机390、白名单保存、切店、空名单/未授权/未登录），并在隐藏内置浏览器插件中检查了场景和弹窗。
- 本地预览 `http://127.0.0.1:5189/erzhuang-project/digitaltwin/` 使用合成接口；不连接真实WSS。真实首帧/声音/截图下载和真实MySQL并发仍需测试环境验收，不能将本地结果称为真实出流通过。
- 本轮未提交或推送，不部署、不修改实例/Secret/流水线。待用户授权测试发布；正式环境不动。

### 2026-09-09 4.0.0 测试发布完成

- 用户授权测试发布后，将功能提交 `50cc856` 备份至 GitHub `codex/digital-twin-module`，再将同一功能提交应用到测试基线 `ab9e4bb`，生成 `3a14913`。保留测试分支专属SSO配置，未合入正式配置及无关历史文档。
- GitLab `codex/containerize-single-image` 正常推送至 `3a1491308b6e3708398fef093f92044d0cae516c`。正式 `main` 只读核对仍为 `bf41906a`，未改动。
- 最终测试分支 Go 全仓测试与 Linux 编译通过；前端与已验证功能分支内容完全一致。
- 浏览器插件确认 Wharf 测试流水线752构建 `171609` 成功（2026-09-09 10:12:40，耗时2分4秒），自动部署记录 `418955` 在10:14:49显示测试环境「已部署」，commit为 `3a149130`。
- 测试入口：https://lite.sy.soyoung.com/erzhuang-project/digitaltwin/ 。本次以Wharf部署记录确认发布；无登录curl健康检查被SSO网关302拦截，真实摄像头首帧与声音仍需页面验收。无DDL、实例配置或正式环境变更。

### 2026-09-09 数字孪生反馈调整（本地4.1.0，未发布）

- 普通治疗室数字自然升序，VIP其次，产研中心等特殊治疗室最后；等候区映射补充候诊区。
- 删除看板外层白色顶部导航，在深色头部姓名后保留原SSO登出逻辑。
- 恢复演示设置，按钮移至大标题右侧；保留各区域人数加减，隐藏模拟权限和JSON调试入口，不改变真实摄像头。人数重绘后保留真实登录标签与演示数据声明。
- 前端123项测试通过，桌面/手机7组浏览器验收覆盖新按钮与人数变化；尚未推送本轮调整到测试分支。

### 2026-09-09 4.1.0 测试部署确认

- 用户授权发布；开发提交 `e11dbab` 已备份 GitHub功能分支，测试提交 `20d5a4c6` 已正常推送 GitLab `codex/containerize-single-image`，保留测试专属配置，正式main未操作。
- 前端生产构建、Go全仓回归通过，最终测试分支frontend与已验收开发分支一致。
- Wharf流水线752构建 `171616` 成功（10:35:06开始，耗时1分43秒），部署 `418995` 在2026-09-09 10:36:57显示测试环境已部署，版本 `20d5a4c6` / 4.1.0。
- 入口仍为 https://lite.sy.soyoung.com/erzhuang-project/digitaltwin/ 。本轮由Wharf部署记录确认上线，业务页面由用户继续验收。

### 2026-09-09 数字孪生全屏黑条修复（本地4.1.1）

- 复现根因：弹窗`.h5-player-container`的`max-height:72vh`在Fullscreen API进入全屏后仍生效；2560×1440时画面高度1036.8px，底部空403.2px。
- 修复仅作用数字孪生弹窗全屏：解除容器高度上限、各层填满全屏；canvas按原比例contain居中，退出全屏恢复弹窗尺寸，不改取流、解码和原监控页面。
- 新增独立合成画面浏览器回归：2560×1440、3840×2160、3440×1440、1440×900全部通过，进入全屏和退出恢复均断言；前端构建通过。未使用真实摄像头，不影响用户浏览器。
- 当前本地4.1.1，未提交推送或部署。

### 2026-09-09 4.1.1 测试部署确认

- 用户授权测试发布；开发提交 `8203aef` 备份至GitHub功能分支，测试提交 `56542f0e` 正常推送到GitLab `codex/containerize-single-image`。仅全屏CSS、回归脚本和VERSION变更。
- Wharf流水线752构建 `171620` 成功（10:47:23开始，耗时1分51秒）；自动部署 `418999` 在10:49:21显示测试环境已部署，版本4.1.1 / `56542f0e`。
- 只读核对正式main仍为 `bf41906a`，未操作正式环境。入口 https://lite.sy.soyoung.com/erzhuang-project/digitaltwin/ ，等待用户实际显示器全屏验收。

### 2026-09-09 4.1.2 Logo与弹窗样式测试部署

- 使用用户提供的627×96透明PNG替换数字孪生左上角Logo，保持比例；播放器弹窗改为与看板一致的深绿底色、细边框和浅色标题，原监控页面不变。
- 前端生产构建、7组看板浏览器检查和4组全屏尺寸回归通过。开发提交 `a2efadf` 已备份GitHub功能分支；测试提交 `35804975` 已正常推送GitLab测试分支。
- 浏览器插件确认Wharf构建 `171628` 成功（10:56:50，耗时1分42秒），部署 `419007` 于10:58:38显示测试环境已部署4.1.2 / `35804975`。正式环境未操作。

### 2026-09-09 4.1.3 关闭按钮悬停修复测试部署

- 根因：公共plain-button悬停文字为深灰，覆盖弹窗默认浅色文字。数字孪生弹窗hover显式设为白色，默认/聚焦仍浅色；7组浏览器回归和前端构建通过。
- 开发提交 `dc264c5` 备份GitHub功能分支；测试提交 `190a7e66` 已推送GitLab测试分支，正式环境未操作。
- Wharf构建 `171641` 成功（11:11:02开始，耗时2分21秒），自动部署 `419039` 于11:13:30显示测试环境已部署4.1.3 / `190a7e66`。

### 2026-09-14 4.4.0 数字孪生实时 RPC 接入准备

- 已把公司提供的 `sdy.proto` 固化到 `proto/soyoung/primecrm/sdy.proto`，协议包名和服务全名保持 `com.soyoung.primecrm.SdyService`；仅将 `go_package` 指向本项目内部包，避免依赖不存在的公司 `go/pb` SDK。
- 新增 `internal/digitaltwin.Service` 和白名单保护接口 `GET /api/digitaltwin/orgs/{externalOrgId}/dashboard`。三路 RPC 在同一 5 秒超时内并发调用；任一路失败时整体返回不可用，不会把失败刷新成 0。前端接口成功时切换左侧指标为 RPC 数据，失败时保留当前看板数据和原摄像头能力。
- 本地验证：数字孪生包 Go 单测通过；`cmd/server` 测试二进制编译通过；`go build ./...` 通过；前端 Vitest `19 files / 123 tests`、生产构建通过；隔离 Chrome 数字孪生验收 `7 passed`。浏览器插件因本机连接器认证方式错误未使用，未触碰用户 Chrome 页面。
- 当时的技术阻塞已解除：生成器源码包和官方 ARM64 `protoc` 已准备完毕，未使用公司 `go/pb` SDK，而是按批准协议在仓库内生成客户端代码。真实测试 Pod 的 ZooKeeper/RPC 连通性仍待部署后验证。

### 2026-09-14 4.4.0 RPC 生成器与适配器完成

- 用户提供的 `protobuf-go-1.26.0.tar.gz` 与 `tools-1.0.2.tar.gz` 均确认可用，已从源码编译为原生 ARM64 工具并安装到忽略目录 `.tools/bin`：`protoc-gen-go v1.26.0`、`protoc-gen-go-triple 1.0.2`；另补充官方 ARM64 `protoc 3.21.12` 到同一目录。
- 已生成并纳入仓库：`internal/digitaltwin/primecrm/sdy.pb.go`、`internal/digitaltwin/primecrm/sdy_triple.pb.go`。生成器 1.0.2 已是旧版 Triple 客户端形态，不使用 `useOldVersion=true`。
...
### 2026-09-15 4.5.0 数字孪生实时指标轮询

- 左侧实时指标在进入页面和切换门店时立即请求，页面可见期间每 30 秒刷新一次；浏览器标签进入后台后暂停，恢复可见时立即刷新并重新开始 30 秒计时。
- 定时刷新只请求 dashboard 指标，不重复拉取机构、摄像头或视频流。刷新失败保留上一轮显示值，并在受影响指标旁显示淡黄色失效点；首次请求失败时保留演示快照但同样标记失效，避免被误认为实时数据。
- 看板实例在刷新时使用 `update` 更新指标，不销毁或重建 iframe 内的场景，避免周期刷新造成画面抖动。轮询单测、前端全量测试、生产构建和隔离无头 Chrome 数字孪生浏览器验收均作为发布门禁。

### 2026-09-15 4.6.0 数字孪生 T+1 BI 30 天趋势测试推送

- 右侧 T+1 指标使用独立 Hprose `dataApi` 接口，按上海时区查询最近 30 个自然日；服务端补齐缺失日期为 `null`，不伪造业务零值。进入页面、刷新和切换门店各重新请求一次；不跟随左侧实时人数的 30 秒轮询，失败时保留本次页面上一轮成功趋势。
- 本地开发提交 `23aba93` 已备份 GitHub 功能分支；隔离测试发布提交 `5d1772c` 从 GitLab 测试基线 `ed44442` 移植，冲突仅涉及说明文案，保留测试分支左侧真实 RPC 实现。正常推送 GitLab `codex/containerize-single-image`，未操作正式 `main`、实例配置、Secret 或数据库。
- 最终测试提交上 Go 全仓测试、Linux amd64 服务构建、前端 `19 files / 128 tests` 与生产构建通过；独立无头 Chrome 用完整 30 天且缺失分组为 `null` 的趋势形态检查 6 张图，无 `NaN` 图形，未触碰用户已有 Chrome 页面。
- 推送后 GitLab 测试分支 HEAD 已复核为 `5d1772c`。Wharf 构建列表匿名 API 返回 `401 need login`，测试入口及 `/health` 匿名请求返回测试 SSO `302`；因此镜像构建与测试实例实际版本仍待有权限的平台或已登录页面确认，不能把代码推送等同于部署成功。不要重复触发构建。

### 2026-09-16 4.7.0 数字孪生验收候选（包含 4.6.3）

- 当前候选分支以 `10f8c58 fix: preserve page after sso timeout redirect (4.6.3)` 为直接基线继续开发；本轮未改动 `frontend/src/domain/auth.ts`、认证回跳单测或会话浏览器验收脚本，确保空闲超时后回到原页面的 4.6.3 修复不会在数字孪生迭代中遗漏。
- 本轮去除可见的演示设置及演示/示意标签，将“预计到店人数”改为“预约暂未到店”；到访人数图支持“全部顾客”兜底，并在上游提供分群数据时只绘制“无需咨询 + 需要面诊”堆叠柱。
- 右侧 T+1 30 天窗口截止上海时区昨天。以 2026-09-16 为当前日期时，查询和展示范围为 `2026-08-17` 至 `2026-09-15`，避免当天未结算数据形成固定空栏。
- 浏览器验收发现并修复异步 T+1 数据未写入新 iframe 快照的问题；独立无头 Chrome 7 组场景通过，桌面与移动截图无横向溢出。前端 `21 files / 132 tests`、生产构建、Go 全仓测试和 Go 全仓构建通过。
- 当前仅形成本地验收候选，尚未推送 GitHub、GitLab 测试分支或正式 `main`。后续测试发布必须从已含 4.6.3 的测试基线继续，业务验收通过后再将同一完整候选提交正式环境。

### 2026-09-16 4.8.0 数字孪生 BI 指标口径与到院路径堆积

- 右侧 T+1 BI 统一业务称谓：`到院人次`、`在店时长`、`升单率`、`核销客单价`、`人均服务点数`；到院、核销客单价和人均服务点数的单位分别调整为 `人次`、`元/人次`、`点/人次`，tooltip 同步使用相同单位。
- 通用图例统一为 `全部顾客 / 非面诊 / 面诊`。到院图在上游同时提供 `visit_front_desk`、`visit_no_consult`、`visit_need_consult` 时，按 `前台签到 / 非面诊 / 面诊` 绘制三段真实堆积，并提示“全部顾客 = 前台签到 + 非面诊 + 面诊”。
- `visit_front_desk` 是可选上游字段，服务端和前端只做原值透传，不允许用“全部顾客 - 非面诊 - 面诊”推算。某一天任一路径未取得数据时，该天只绘制 `visit_all` 的“全部顾客”单柱；数值 `0` 仍作为有效数据，与 `null` / 未取得数据严格区分。
- 验证结果：前端 Vitest `22 files / 134 tests` 通过，生产构建通过；`cmd/server` 与 `internal/digitaltwin/t1bi` 测试目标编译通过，Linux amd64 服务构建通过；独立无头 Chrome 在桌面、笔记本和手机宽度完成 `7` 组图表验收，覆盖完整三段堆积、单日总数回退、动态图例、新单位 tooltip 和横向溢出检查，未操作用户正在使用的浏览器。
- 当前仍依赖数据部门在 T+1 返回中正式提供 `visit_front_desk`。字段尚未提供的历史日期会按已确认方案展示“全部顾客”单柱，不伪造前台签到数据。
