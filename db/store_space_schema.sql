...
create table if not exists store_areas (
  ...
  constraint store_areas_source_check check (source in ('manual', 'design_plan', 'video_channel', 'multiple')),
  constraint store_areas_status_check check (status in ('candidate', 'confirmed')),
  constraint store_areas_area_number_check check (area_number > 0)
);
...
create table if not exists design_plan_annotations (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint design_plan_annotations_status_check check (status in ('pending', 'confirmed')),
...
create table if not exists ezviz_accounts (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint ezviz_accounts_status_check check (status in ('unverified', 'available', 'unavailable'))
);
...
create table if not exists video_recorders (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint video_recorders_status_check check (status in ('online', 'offline')),
  constraint video_recorders_effective_channel_count_check check (effective_channel_count >= 0)
);
...
create table if not exists video_channels (
  ...
  constraint video_channels_channel_no_check check (channel_no > 0),
  constraint video_channels_recognition_attempts_check check (recognition_attempts >= 0)
);
...
create policy stores_no_client_access on stores for all to anon, authenticated using (false) with check (false);
drop policy if exists store_areas_no_client_access on store_areas;
create policy store_areas_no_client_access on store_areas for all to anon, authenticated using (false) with check (false);
drop policy if exists store_design_plans_no_client_access on store_design_plans;
create policy store_design_plans_no_client_access on store_design_plans for all to anon, authenticated using (false) with check (false);
drop policy if exists design_plan_annotations_no_client_access on design_plan_annotations;
create policy design_plan_annotations_no_client_access on design_plan_annotations for all to anon, authenticated using (false) with check (false);
drop policy if exists ezviz_accounts_no_client_access on ezviz_accounts;
create policy ezviz_accounts_no_client_access on ezviz_accounts for all to anon, authenticated using (false) with check (false);
drop policy if exists video_recorders_no_client_access on video_recorders;
create policy video_recorders_no_client_access on video_recorders for all to anon, authenticated using (false) with check (false);
drop policy if exists video_channels_no_client_access on video_channels;
create policy video_channels_no_client_access on video_channels for all to anon, authenticated using (false) with check (false);
drop policy if exists channel_snapshots_no_client_access on channel_snapshots;
create policy channel_snapshots_no_client_access on channel_snapshots for all to anon, authenticated using (false) with check (false);
drop policy if exists operation_logs_no_client_access on operation_logs;
create policy operation_logs_no_client_access on operation_logs for all to anon, authenticated using (false) with check (false);
