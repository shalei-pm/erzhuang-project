...
## 2026-08-26 增量：3.1.12 NVR 回放进度定位

- [x] 产品反馈：NVR 回放模式缺少 2.x 既有的播放器内进度拖动。
- [x] 设计：页面级底栏继续仅负责实时/录像模式切换；播放器中央复用 2.x `PlaybackSegmentSlider`，只在已创建 NVR 回放会话时显示。
- [x] 行为：当前小时回放窗口作为 slider 范围；拖动松开后，以目标秒作为新的开始时间、原窗口结束时间不变，重新创建回放会话。
- [x] 约束：工控机 NVR SDK 没有可用媒体时间码，因此首帧后按播放器实际播放/暂停状态做秒级游标估算；拖动仍以服务端重新签发的回放会话为准。
- [x] 前端回归：Vitest 10 个文件 / 60 项测试与生产构建通过。
- [x] Chrome 本地预览确认页面底栏仍独立于播放器，播放器内没有模式导航；本地 API 未接公司后端，HTTP 500 环境不能伪造真实回放会话。
- [x] 已提交 `78e0a30`，同步 GitHub 与 GitLab 测试分支，Wharf `752` 自动部署生效；一次性 GitLab 凭据脚本已删除。
- [x] Chrome 测试环境：`camera_id=111` 昨天 `11:00 - 12:00` 回放首帧成功；滑块最大值为 3600，游标持续推进，重新定位后从目标位置重新创建会话并恢复播放；控制台无项目 error。
- [ ] 保持隔离实验页范围，后续补 iPhone Safari 与微信内直播真机验收；旧萤石 H5 Monitor 与正式环境不随本轮变更。

## 2026-08-26 修复：NVR 回放滑块定位比例

- [x] 复现：任意位置拖动后，滑块区间会随目标起点缩短，导致视觉位置错误。
- [x] 根因：同一个 `activePlaybackRange.startTime` 同时承担固定小时窗口与 WSS 会话起点两个职责。
- [x] 修复：新增回放会话模型，保持小时窗口稳定，只将目标秒传给后端 `start_time`。
- [x] 回归：新增窗口保持测试；Vitest 10 个文件 / 61 项测试与生产构建通过。
- [x] 已发布测试环境 `1704b0a`；Chrome 真实验证中段定位保持 `1800 / 3600`、末段定位保持 `3264 / 3600`，两次均恢复首帧并继续推进，控制台无项目 error。

## 下一阶段：10001 灰度决策

- [x] 用户阶段确认：10001 实验页的样式、核心功能和实际使用已基本可用。
- [ ] 决定是否继续保持 10001 隔离实验页，或制定替换旧监控链路的灰度与回滚方案。
- [ ] 如扩大灰度，先确认目标门店、功能范围、观测指标、失败回退和正式发布门槛；正式环境在该决策前不改。

## 待设计：工控机监控全量替换

- [x] 目标确认：正常“查看监控”入口全量切换至工控机 NVR，旧萤石不再作为日常入口。
- [x] 门店准入确认：没有符合工控机摄像头口径的门店不展示入口，不自动回退旧链路。
- [x] 权限确认：管理员、编辑运维看全部已配置门店；普通查看仅看自身门店授权范围。
- [x] 回滚确认：新入口不展示旧萤石，但保留旧代码/数据配置作为版本回滚点。
- [ ] 提出并确认全量架构设计，再写实施计划；确认前不改业务代码。

## 2026-08-26 已确认设计：工控机监控全量替换

- [x] 用户确认方案 A：替换正常 H5 监控路由，未配置门店不展示入口，旧萤石不在新 UI 暴露。
...
- [x] 缩略图首步：NVR 监控列表渲染已有、安全匹配的历史截图；无法可靠匹配或图片加载失败时使用中性灰色图片占位。`3.2.1` 前端测试 `11 files / 64 tests` 和生产构建通过，待测试环境真实验收。
- [x] `3.2.1` 测试环境验收：Wharf 构建 `169173` 成功并自动部署为 `414738`；Chrome 验证 `10001` 的 44 路摄像头从黑色块变为中性灰色图片占位，未见项目控制台错误。该门店无可安全复用历史图，真实图片覆盖率待以有旧截图样本的门店补验。
- [ ] 待决策：缩略图第二阶段。用户打开且首帧成功后，由前端截 Canvas 并经二壮后端写入私有 OSS，逐步补齐；或由工控机平台提供受鉴权静态截图 API。未确认前不新增截图写入、后台抓流或长期媒体留存。
- [x] 方向纠正：缩略图初始化不做普通用户功能、单摄像头手工维护或浏览器驻留队列。用户确认将其视为一次性的后端数据回填工作；页面只读取回填结果。待设计独立、严格串行的后台回填任务，不在二壮 Web 实例增加运行配置。
- [x] 后端回填设计已完成并待用户审阅：`docs/superpowers/specs/2026-08-26-nvr-snapshot-backfill-design.md`。确认后才编写实施计划、DBA DDL 提案和 `10001` 单摄像头技术验证。
...
- [ ] DBA/运维先审批并执行自有表 `tb_nvr_camera_snapshots` 的测试库 DDL；Web 服务不得自行建表。
- [ ] 先对 `10001` 的一个已知直播可用摄像头进行原生 WSS/RTP/H.265 到 ffmpeg/JPEG 的 20 秒、并发 1 技术闸门。失败即停止，不进入 44 路或全量批处理。
- [ ] 技术闸门通过后才实现和执行严格串行 Job；`10001` 批量、全量测试、生产执行均需逐级单独确认。
