...
- [ ] 待决策：NVR 监控列表缩略图。当前黑色画面是前端统一空态，并未请求工控机截图；旧 2.x 截图只能在单录像机门店按通道号作受限历史兜底。候选路径为：A. 低成本复用可安全匹配的历史截图，其余保持空态；B. 用户打开且首帧成功后，由前端截 Canvas 并经二壮后端写入私有 OSS，逐步补齐；C. 由工控机平台提供受鉴权的静态截图接口，二壮只读代理/缓存。未确认前不修改缩略图代码或新增存储。
