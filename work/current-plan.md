...
## 下一阶段：10001 移动端门槛与灰度决策

- [x] 用户阶段确认：桌面端的页面样式与核心功能框架基本定稿。
- [ ] 在 iPhone Safari 验证 `10001 / camera_id=111` 直播首帧、暂停/恢复、声音与横屏。
- [ ] 在微信内验证同一直播路径与回退提示。
- [ ] 根据真机结果决定继续保持 10001 隔离实验页，或制定替换旧监控链路的灰度与回滚方案；正式环境在该决策前不改。
