...
export function digitalTwinDocument() {
  ...
  override.textContent = ".scene-modebar,.permission-control,.kit-input-panel,.debug-toggle,.debug-panel,.experiment-stores small,.store-picker .sample-tag,.bi-heading .sample-tag,.bi-disclaimer,.scene-footer>span:first-child{display:none!important}.brand{pointer-events:none}.scene-footer{flex-wrap:wrap}.header-meta{gap:12px}body{margin:0}.product-name{display:grid;align-items:center;gap:0 12px}.product-name>span{font-size:10px;letter-spacing:0;color:#b6c9bd}.product-name h1{letter-spacing:0}@media(max-width:1099px){body.dashboard{min-height:0}}@media(max-width:650px){.product-name{gap:0 6px}}";
  ...
  doc.querySelector(".product-name>span")!.textContent = "STORE DIGITAL TWIN";
