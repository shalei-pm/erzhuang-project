(function(root){
 ...
 const chartSpecs=[
  ...
  {id:'redemption',title:'人均核销金额',en:'REDEMPTION PER GUEST',type:'line',unit:'元/人',max:2000,series:[series('全部顾客','redemptionAll',colors.all),series('无需咨询','redemptionNo',colors.no),series('需要咨询','redemptionConsult',colors.consult)]}
 ];
 function buildDemoData(){
  ...
  return Array.from({length:30},(_,i)=>{
   ...
   return {date:new Date(Date.UTC(2026,7,10+i)).toISOString().slice(0,10),noConsult,consult,stayNo,stayConsult,stayAll:round((stayNo*noConsult+stayConsult*consult)/(noConsult+consult)),waitNo,waitConsult,waitAll:round((waitNo*noConsult+waitConsult*consult)/(noConsult+consult)),upgradeNo,upgradeConsult,upgradeAll,redemptionNo,redemptionConsult,redemptionAll};
  });
 }
