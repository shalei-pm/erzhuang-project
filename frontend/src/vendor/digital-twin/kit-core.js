...
(function(root){
 ...
 const trendKeys=['noConsult','consult','stayAll','stayNo','stayConsult','waitAll','waitNo','waitConsult','upgradeAll','upgradeNo','upgradeConsult','redemptionAll','redemptionNo','redemptionConsult'];
 ...
 function validate(s){
  ...
  for(const id of regionIds){const r=s.regions[id];keys(r,['current','cumulative','staff','cameras'],`regions.${id}`);for(const k of ['current','cumulative','staff'])count(r[k],`${id}.${k}`);
 ...
 function merge(current,patch){
  keys(patch,['mode','revision','updatedAt','store','overview','permissions','regions','trends'],'data');const next=clone(current);
  ...
  if(Object.hasOwn(patch,'regions')){keys(patch.regions,regionIds,'regions');for(const [id,value]of Object.entries(patch.regions)){keys(value,['current','cumulative','staff','cameras'],`regions.${id}`);Object.assign(next.regions[id],clone(value));}}
