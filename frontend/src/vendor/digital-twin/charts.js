...
(function(root){
 ...
 function render(container,data,{mode='external',specs=TwinChartData.chartSpecs,rotationMs=7000}={}){
  ...
  for(const [slotIndex,slot]of slots.entries())for(const spec of slot){
   ...
   const visibleSeries=spec.series.filter(series=>data.some(datum=>TwinChartData.seriesForDatum(spec,datum).some(item=>item.key===series.key)));
   const legend=visibleSeries.map(s=>`<span><i style="--series-color:${s.color}" class="${spec.type==='stacked'?'bar-key':'line-key'}"></i>${esc(s.label)}</span>`).join('');
   const breakdownNote=spec.stackedBreakdown?.note?`<span class="chart-breakdown-note">${esc(spec.stackedBreakdown.note)}</span>`:'';
   ...
   const demoLabel=mode==='demo'?'模拟数据':'';
   const accessibilitySuffix=demoLabel?`，${demoLabel}`:'';
   card.innerHTML=`<div class="chart-heading"><div><span class="chart-index">${String(slotIndex+1).padStart(2,'0')}</span><h3 id="chart-title-${spec.id}">${esc(spec.title)}</h3></div><span class="chart-unit">${esc(spec.unit)}</span></div><div class="chart-legend">${legend}<span class="chart-time">${length}天</span>${breakdownNote}</div><div class="chart-canvas"><svg class="trend-svg" viewBox="0 0 ${WIDTH} ${HEIGHT}" preserveAspectRatio="none" role="img" tabindex="0" aria-label="${esc(spec.title)}，${length}天${accessibilitySuffix}；左右方向键查看每日数值"><title>${esc(spec.title)}${accessibilitySuffix}</title>${shapes}<line class="chart-crosshair" x1="0" y1="${Y0}" x2="0" y2="${Y1}" stroke="#a4c9b2" stroke-width="1" stroke-dasharray="3 3" visibility="hidden"/></svg><div class="chart-tooltip" hidden></div>${hasValue?'':'<div class="chart-empty">暂无数据</div>'}</div>`;
   ...
   function show(index){if(!length)return;current=Math.max(0,Math.min(last,index));const d=data[current];tooltip.replaceChildren();const title=document.createElement('strong');title.textContent=`${d.date.replaceAll('-','.')}${mode==='demo'?' · 模拟':''}`;tooltip.append(title);
