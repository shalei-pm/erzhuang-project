...
func TestAPISIXSSOLogoutGetSupportsProductionDomain(t *testing.T) {
	t.Setenv("APP_BASE_PATH", "/erzhuang-project")
	t.Setenv("SSO_ENABLED", "true")

	gatewayLogout := "https://security-test.sy.soyoung.com/api/g/sso/logouttogether?from_host=lite.soyoung.com&from_uri=http%3A%2F%2Flite.soyoung.com%2Ferzhuang-project%2F"
	...
	if !hasClearedAuthCookie(cookies, "sy_sso_token", "lite.soyoung.com") {
		t.Fatalf("expected logout to clear host sso cookie, got %#v", cookies)
	}
	if !hasClearedAuthCookie(cookies, "sy_sso_token", "soyoung.com") {
		t.Fatalf("expected logout to clear parent-domain sso cookie, got %#v", cookies)
	}
}
